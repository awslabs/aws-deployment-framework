...
def is_applicable_ec2(configuration_item, event):
    ...
    # Fetch about autoscaling 
    if is_part_of_asg(configuration_item['resourceId'], event):
        return False 
...
def is_termination_protection_on(instance_id, event):
    ...
    response = client.describe_instance_attribute(Attribute='disableApiTermination', InstanceId=instance_id)
