...
def describe_regions(role):
    ...
    try:
        ...
        if err.response["Error"]["Code"] == 'OptInRequired': # This error is raised when the account is not yet ready but describe_regions is called
            sleep(20)
            return describe_regions(role)
...
def create_or_update_account(org_session, account, adf_role_name, account_id=None):
    ...
    """
    if not account_id:
        LOGGER.info(f'Creating new AWS Account {account.full_name}')
    ...
    if account.delete_default_vpc:
        LOGGER.info(f'Delete Default VPC is set to True for {account_id} (alias {account.alias}) is in OU {account.ou_path}')
        all_regions = describe_regions(role)
        args = (
