...
def test_map_state_to_legacy_status_known_states(state, expected_status):
    assert roa._map_state_to_legacy_status(state) == expected_status
...
def test_map_state_to_legacy_status_unknown_states(state):
    assert roa._map_state_to_legacy_status(state) is None


def test_state_to_legacy_status_map_covers_all_documented_states():
    # Guard against silently dropping one of the documented State values.
    assert set(roa.STATE_TO_LEGACY_STATUS_MAP) == {
        "PENDING_ACTIVATION",
        "ACTIVE",
        "SUSPENDED",
        "PENDING_CLOSURE",
