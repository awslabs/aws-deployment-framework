"""
Generates the standard adf pipeline.
"""
import os
...
def generate_adf_pipeline(scope: core.Stack, _build_name, _source_name, stack_input) -> None: #pylint: disable=R0912, R0915
    ...
    if "codebuild" in _build_name and stack_input["input"]["default_providers"]["build"].get("properties", {}).get(
            "enabled", True):
        _stages.append(
    ...
    for index, targets in enumerate(stack_input["input"].get("environments", {}).get("targets", [])):
        ...
        top_level_deployment_type = (
            stack_input["input"].get("default_providers", {}).get("deploy", {}).get("provider", "") or "cloudformation"
        )
        top_level_action = (
            stack_input["input"].get("default_providers", {}).get("deploy", {}).get("properties", {}).get("action", "")
        )
                    ...
                    [
                        adf_codebuild.CodeBuild(
                            ...
                            "{pipeline_name}-stage-{index}".format(
                                pipeline_name=stack_input["input"]["name"], index=index + 1,
                            ),
                            stack_input["ssm_params"][ADF_DEPLOYMENT_REGION]["modules"],
                            stack_input["ssm_params"][ADF_DEPLOYMENT_REGION]["kms"],
                            stack_input["input"],
        ...
        _is_approval = targets[0].get("name", "").startswith("approval") or targets[0].get("provider", "") == "approval"
        ...
        _stage_name = (
            ...
            or "{action_type_name}-stage-{index}".format(action_type_name=_action_type_name, index=index + 1,)
        )
        _stages.append(_codepipeline.CfnPipeline.StageDeclarationProperty(name=_stage_name, actions=_actions,))
