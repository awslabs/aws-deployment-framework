...
class Action:
    ...
    def _generate_role_arn(self):
        ...
        specific_role = self.target.get("properties", {}).get(
            "role"
        ) or default_provider.get("properties", {}).get("role")
        if specific_role:
            account_id = (
                self.account_id if self.provider == "CodeBuild" else self.target["id"]
            )
    ...
    def _generate_configuration(self):  # pylint: disable=R0912, R0911, R0915
        if self.provider == "Manual" and self.category == "Approval":
            _props = {
                "CustomData": self.target.get("properties", {}).get("message")
                or "Approval stage for {0}".format(self.map_params["name"])
            }
            ...
            if self.target.get("properties", {}).get("sns_topic_arn"):
                _props["NotificationArn"] = self.target.get("properties", {}).get(
                    "sns_topic_arn"
                )
            return _props
        if self.provider == "S3" and self.category == "Source":
            return {
                "S3Bucket": self.map_params.get("default_providers", {})
                .get("source", {})
                .get("properties", {})
                .get("bucket_name"),
                "S3ObjectKey": self.map_params.get("default_providers", {})
                .get("source", {})
                .get("properties", {})
                .get("object_key"),
                "PollForSourceChanges": self.map_params.get("default_providers", {})
                .get("source", {})
                .get("properties", {})
        ...
        if self.provider == "S3" and self.category == "Deploy":
            return {
                "BucketName": self.map_params.get("default_providers", {})
                .get("deploy", {})
                .get("properties", {})
                .get("bucket_name")
                or self.target.get("properties", {}).get("bucket_name"),
                "Extract": self.map_params.get("default_providers", {})
                .get("deploy", {})
                .get("properties", {})
                .get("extract")
                or self.target.get("properties", {}).get("extract", False),
                "ObjectKey": self.map_params.get("default_providers", {})
                .get("deploy", {})
                .get("properties", {})
                .get("object_key")
                or self.target.get("properties", {}).get("object_key"),
            }
        if self.provider == "CodeStarSourceConnection":
            ...
            try:
                ...
                LOGGER.error(
                    f"No parameter found at {codestar_connection_path}. Check the path/value."
                )
                raise e
            ...
            return {
                ...
                "BranchName": self.map_params.get("default_providers", {})
                .get("source", {})
                .get("properties", {})
                .get("branch", {})
                or "master",
        ...
        if self.provider == "GitHub":
            return {
                "Owner": self.map_params.get("default_providers", {})
                .get("source")
                .get("properties", {})
                .get("owner", {}),
                "Repo": self.map_params.get("default_providers", {})
                .get("source", {})
                .get("properties", {})
                .get("repository", {})
                or self.map_params["name"],
                "Branch": self.map_params.get("default_providers", {})
                .get("source", {})
                .get("properties", {})
                .get("branch", {})
                or "master",
                ...
                "OAuthToken": core.SecretValue.secrets_manager(
                    ...
                    json_field=self.map_params["default_providers"]["source"]
                    .get("properties", {})
                    .get("json_field"),
                ),
        ...
        if self.provider == "Lambda":
            return {
                "FunctionName": self.map_params.get("default_providers", {})
                .get("deploy", {})
                .get("properties", {})
                .get("function_name", "")
                or self.target.get("properties", {}).get("function_name", ""),
                "UserParameters": str(
                    self.map_params.get("default_providers", {})
                    .get("deploy", {})
                    .get("properties", {})
                    .get("input", "")
                    or self.target.get("properties", {}).get("input", "")
                ),
            }
        if self.provider == "CloudFormation":
            ...
            _props = {
                ...
                "RoleArn": "arn:aws:iam::{0}:role/adf-cloudformation-deployment-role".format(
                    self.target["id"]
                )
            ...
            if (
                ...
                _props[
                    "TemplatePath"
                ] = "{input_artifact}::{path_prefix}template_{region}.yml".format(
                    input_artifact=_input_artifact,
                    path_prefix=_path_prefix,
                    region=self.region,
                )
                ...
                _template_filename = (
                    self.target.get("properties", {}).get("template_filename")
                    or self.map_params.get("default_providers", {})
                    .get("deploy", {})
                    .get("properties", {})
                ...
                _props[
                    "TemplatePath"
                ] = "{input_artifact}::{path_prefix}{filename}".format(
                    input_artifact=_input_artifact,
                    path_prefix=_path_prefix,
            ...
            if self.target.get("properties", {}).get("param_overrides"):
                ...
                for override in self.target.get("properties", {}).get(
                    ...
                    _overrides["{0}".format(override["param"])] = {
                        "Fn::GetParam": [
        ...
        if self.provider == "Jenkins":
            return {
                "ProjectName": self.map_params["default_providers"]["build"]
                .get("properties", {})
                .get(
                    "project_name", self.map_params["name"]
                ),  # Enter the name of the project you created in the Jenkins plugin
                "ServerURL": self.map_params["default_providers"]["build"]
                .get("properties", {})
                .get("server_url"),  # Server URL
                "ProviderName": self.map_params["default_providers"]["build"]
                .get("properties", {})
                .get(
                    "provider_name"
                ),  # Enter the provider name you configured in the Jenkins plugin
            }
        ...
        if self.provider == "ServiceCatalog":
            return {
                "ConfigurationFilePath": self.target.get("properties", {}).get(
                    "configuration_file_path"
                )
                or "params/{0}_{1}.json".format(self.target["name"], self.region),
                "ProductId": self.target.get("properties", {}).get("product_id")
                or self.map_params["default_providers"]["deploy"]
                .get("properties", {})
                .get(
                    "product_id"
                ),  # product_id is required for Service Catalog, meaning the product must already exist.
            }
        if self.provider == "CodeDeploy":
            return {
                "ApplicationName": self.map_params.get("default_providers", {})
                .get("deploy", {})
                .get("properties", {})
                .get("application_name", {})
                or self.target.get("properties", {}).get("application_name"),
                "DeploymentGroupName": self.map_params.get("default_providers", {})
                .get("deploy", {})
                .get("properties", {})
                .get("deployment_group_name", {})
                or self.target.get("properties", {}).get("deployment_group_name"),
            }
        if self.provider == "CodeCommit":
            return {
                "BranchName": self.map_params["default_providers"]["source"]
                .get("properties", {})
                .get("branch", "master"),
                "RepositoryName": self.map_params["default_providers"]["source"]
                .get("properties", {})
                .get("repository", {})
                or self.map_params["name"],
                "PollForSourceChanges": (
                    ...
                    .get("properties", {})
                    .get("trigger_on_changes", True)
                    and self.map_params["default_providers"]["source"]
                    .get("properties", {})
    ...
    def _generate_codepipeline_access_role(self):  # pylint: disable=R0911
        if self.provider == "CodeCommit":
            return "arn:aws:iam::{0}:role/adf-codecommit-role".format(
                self.map_params["default_providers"]["source"]["properties"][
                    "account_id"
                ]
            )
        if self.provider == "GitHub":
        ...
        if self.provider == "S3" and self.category == "Deploy":
            # This could be changed to use a new role that is bootstrapped, ideally we rename adf-cloudformation-role to a generic deployment role name
            return "arn:aws:iam::{0}:role/adf-cloudformation-role".format(
                self.target["id"]
            )
        if self.provider == "ServiceCatalog":
            # This could be changed to use a new role that is bootstrapped, ideally we rename adf-cloudformation-role to a generic deployment role name
    ...
    def _get_base_input_artifact_name(self):
        ...
        """
        use_output_source = not self.target or not self.map_params.get(
            "default_providers", {}
        ).get("build", {}).get("enabled", True)
        ...
        adf_events.Events(
            ...
            "events",
            {
                "pipeline": "arn:aws:codepipeline:{0}:{1}:{2}".format(
                    ...
                    "{0}{1}".format(
                        os.environ.get("ADF_PIPELINE_PREFIX"), map_params["name"]
                    ),
                ),
                ...
                "source": {
                    "provider": map_params.get("default_providers", {})
                    .get("source", {})
                    .get("provider"),
                    "account_id": map_params.get("default_providers", {})
                    .get("source", {})
                    .get("properties", {})
                    .get("account_id"),
                    "repo_name": map_params.get("default_providers", {})
                    .get("source", {})
                    .get("properties", {})
                    .get("repository")
                    or map_params["name"],
                    "branch": map_params.get("default_providers", {})
                    .get("source", {})
                    .get("properties", {})
                    .get("branch", "master"),
                    "poll_for_changes": map_params.get("default_providers", {})
                    .get("source", {})
                    .get("properties", {})
                    .get("poll_for_changes", False),
                    "trigger_on_changes": map_params.get("default_providers", {})
                    .get("source", {})
                    .get("properties", {})
                    .get("trigger_on_changes", True),
                },
