...
class OrganizationPolicy:
    ...
    def _find_all(policy):
        return [p for p in glob.iglob('./**/{0}.json'.format(policy), recursive=True)]
    ...
    def _trim_tagging_policy_file_name(policy):
        return policy[15:][:-19] if policy[15:][:-19] == '/' else policy[16:][:-20]
    ...
    def set_scp_attachment(access_identifer, organization_mapping, path, organizations):
        if access_identifer:
            if access_identifer.get('keep-default-scp') != 'enabled':
                try:
                    organizations.detach_policy('p-FullAWSAccess', organization_mapping[path])
                ...
                try:
                    organizations.attach_policy('p-FullAWSAccess', organization_mapping[path])
    ...
    @staticmethod
    def clean_and_remove_policy_attachment(organization_mapping, path, organizations, policy_type):
        policy_id = organizations.describe_policy_id_for_target(organization_mapping[path], policy_type)
        ...
        LOGGER.info('Policy (%s) %s will be deleted. Path is: %s', policy_type, organization_mapping[path], path)
    ...
    def apply(self, organizations, parameter_store, config): #pylint: disable=R0912, R0915
        ...
        if status.get('feature_set') != 'ALL':
            LOGGER.info('All Features are currently NOT enabled for this Organization, this is required to apply SCPs or Tagging Policies')
            return

        LOGGER.info('Determining if Organization Policy changes are required. (Tagging or Service Controls)')
        ...
        for policy in ['scp', 'tagging-policy']:
            ...
            try:
                ...
                for stored_policy in current_stored_policy:
                    path = OrganizationPolicy._trim_scp_file_name(stored_policy) if policy == 'scp' else OrganizationPolicy._trim_tagging_policy_file_name(stored_policy)
                    ...
                    if stored_policy not in _policies:
                        OrganizationPolicy.clean_and_remove_policy_attachment(organization_mapping, path, organizations, _type)
            except ParameterNotFoundError:
                LOGGER.debug('Parameter %s was not found in Parameter Store, continuing.', policy)
                pass

            for _policy in _policies:
                path = OrganizationPolicy._trim_scp_file_name(_policy) if policy == 'scp' else OrganizationPolicy._trim_tagging_policy_file_name(_policy)
                policy_id = organizations.describe_policy_id_for_target(organization_mapping[path], _type)
                proposed_policy = Organizations.get_policy_body(_policy)
                if policy_id:
                    current_policy = organizations.describe_policy(policy_id)
                    if self._compare_ordered_policy(current_policy.get('Content')) == self._compare_ordered_policy(proposed_policy):
                        LOGGER.info('Policy (%s) %s does not require updating. Path is: %s', policy, organization_mapping[path], path)
                        continue
                    LOGGER.info('Policy (%s) will be updated for %s. Path is: %s', policy, organization_mapping[path], path)
                ...
                try:
                    ...
                    LOGGER.info('Policy (%s) has been created for %s. Path is: %s', policy, organization_mapping[path], path)
                    ...
                    LOGGER.info('Policy (%s) for %s already exists but was not attached, attaching.', policy, organization_mapping[path])
                    policy_id = organizations.list_policies('adf-{0}-{1}'.format(policy, path), _type)
                    organizations.attach_policy(policy_id, organization_mapping[path])
