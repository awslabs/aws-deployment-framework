...
- *poll_for_changes* - *(Boolean)* default: `False`.
  ...
  False in favor of Amazon EventBridge events. As the name implies, when polling
  for changes it will check the repository for updates every minute or so. This
  will show up as actions in CloudTrail.
...
- *trigger_on_changes* - *(Boolean)* default: `True`.
  ...
  disables the triggering of changes all together when set to False.
...
S3 can use used as the source for a pipeline too. **Please note:** you can use
...
- *repository* - *(String)* defaults to name of the pipeline.
  ...
  For example, for the ADF repository it would be `aws-deployment-framework`.
- *branch* - *(String)* default to configured [adfconfig.yml: config/scm/default-scm-branch](./admin-guide.md#adfconfig).
...
- *codestar_connection_path* - *(String)* **(required)**
  ...
  web hook as part of the pipeline. Read the CodeStar Connections documentation
...
- *environment_variables* *(Object)* defaults to empty object.
  ...
  for this pipeline. These are to be passed in as Key/Value pairs. For example:
...
- *role* - *(String)* default `arn:${partition}:iam::${target_account_id}:role/adf-cloudformation-role`.
  ...
  it. As is
  [documented in the CodeDeploy service role documentation](https://docs.aws.amazon.com/codedeploy/latest/userguide/getting-started-create-service-role.html).
...
- *stack_name* - *(String)* default: `${ADF_STACK_PREFIX}${PIPELINE_NAME}`.
  - The name of the CloudFormation Stack to use. Yhe default `ADF_STACK_PREFIX`
  is `adf-`. This is configurable as part of the `StackPrefix` parameter in the
  `deployment/global.yml` stack.
...
- *action* - (`CHANGE_SET_EXECUTE|CHANGE_SET_REPLACE|CREATE_UPDATE|DELETE_ONLY|REPLACE_ON_FAILURE`)
...
- *input* *(Object|List|String)* defaults to empty string.
  - An object to pass into the Lambda function as its input event.This input
